...
echo Step 1: Starting SearchAPI instances...
echo ----------------------------------------

echo Starting SearchAPI Instance 1 (Port 5154)...
start "SearchAPI Instance 1" cmd /k "cd /d SearchAPI && dotnet run --launch-profile Ole"

timeout /t 5 /nobreak > nul

echo Starting SearchAPI Instance 2 (Port 5155)...
start "SearchAPI Instance 2" cmd /k "cd /d SearchAPI && dotnet run --launch-profile Henrik"
...
start "Load Balancer" cmd /k "cd /d Loadbalencer && dotnet run"

timeout /t 5 /nobreak > nul

echo.
echo Step 3: Starting Web Applications...
echo -------------------------------------

echo Starting SearchWeb...
start "SearchWeb" cmd /k "cd /d SearchWeb && dotnet run"
